...
import { Search, User, Clock, ThumbsUp, ThumbsDown, Calendar, X, ChevronDown} from "lucide-react";
...
const QueryAnalytics = ({ data }: { data: DashboardData | null }) => {
  ...
  const ratingStats = useMemo(() => {
    ...
    return [
      { value: "ALL", count: allQueries.length },
    ...
    <div className="max-w-none mx-auto py-8 px-4">
      ...
      <div className="flex items-center justify-between gap-3 mb-5 flex-wrap">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-[11px] font-mono text-muted-foreground bg-secondary border border-border px-2 py-0.5 rounded">
            ...
            <div className="flex justify-between items-start gap-3 mb-1.5">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1.5">
                  ...
                  <span className="text-[10px] font-mono text-muted-foreground/60 truncate uppercase">
                    {q.query_id}
                  </span>
                </div>
              ...
              <div className="shrink-0 pt-1">
                {q.rating === "like" && <ThumbsUp className="w-4 h-4 text-emerald-500" />}
                {q.rating === "dislike" && <ThumbsDown className="w-4 h-4 text-rose-500" />}
              </div>
            </div>

            <div className="flex items-center justify-between mt-2.5 text-[10px] font-mono text-muted-foreground/60">
              <span className="truncate max-w-[180px] bg-secondary/50 px-1.5 py-0.5 rounded border border-border/40">
                {q.userEmail}
              </span>
              <span className="flex items-center gap-1">
                <Calendar className="w-2.5 h-2.5" /> 
                {q.session_created ? new Date(q.session_created).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : "-"}
              </span>
            </div>
          </button>
        ))}
  ...
  // Detail Panel (unchanged - you can keep your existing detailConfig)
  const detailConfig = selectedQuery ? {
    header: (
      <div className="flex items-start gap-2.5 px-4 py-3.5 relative overflow-hidden">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1.5">
            <div className="px-1.5 py-0.5 rounded border bg-blue-400/10 border-blue-400/30 text-[9px] font-mono text-blue-400 uppercase tracking-widest">
              Query Analytics
            </div>
